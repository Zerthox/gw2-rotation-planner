...
export const findRow = (rows: RowState[], id: DragId): RowState =>
    rows.find((row) => row.dragId === id);

export const findRowIndex = (rows: RowState[], id: DragId): number =>
    rows.findIndex((row) => row.dragId === id);

export type RowAction<Data = unknown> = PayloadAction<{ rowId: DragId } & Data>;

export const timelineSlice = createSlice({
    name: "timeline",
    initialState: {
        rows: [createRow({ name: "", skills: [] })],
    },
    reducers: {
        ...
        insertRowWithStates(
            state,
            { payload }: PayloadAction<{ index?: number; row: Partial<RowWithoutId> }>,
        ) {
            const { index, row } = payload;
...
export const selectStatelessRows = createSelector(selectRows, (rows) =>
    rows.map(({ name, skills }) => ({
        name,
