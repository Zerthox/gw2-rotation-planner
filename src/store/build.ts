...
const filterSections = (sections: SkillSection[], prof: Profession) =>
    sections.filter((section) => !section.profession || section.profession === prof);
...
export const buildSlice = createSlice({
    ...
    reducers: {
        ...
        refreshSkillStates(state) {
            state.skillStates = filterSections(state.sections, state.profession).map((section) =>
                section.skills.map((skill) => createSkillState(skill)),
            );
        },
...
export const selectCurrentProfession = ({ buildReducer }: StoreState): Profession =>
    buildReducer.profession;
...
export const selectAllSections = ({ buildReducer }: StoreState): SkillSection[] =>
    buildReducer.sections;
...
export const selectSkillStates = ({ buildReducer }: StoreState): SkillState[][] =>
    buildReducer.skillStates;
