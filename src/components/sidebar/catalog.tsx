...
const SkillSection = ({ name, type, parentId, skills, sx }: SkillSectionProps): JSX.Element => {
    ...
    const isOrdered =
        ...
        && (type === SkillSectionType.Weapon || type === SkillSectionType.Bundle);
    ...
    return (
        <Accordion
            ...
            sx={sx}
        >
            <AccordionSummary expandIcon={<ExpandMore />} aria-controls={`${name} Skills Panel`}>
...
export const SkillCatalog = ({ dragId, ...props }: SkillbarProps): JSX.Element => {
    ...
    return (
        <Stack direction="column" sx={{ overflowY: "auto" }} {...props}>
            {sections.map(({ name, type, skills }, i) =>
                skills.length > 0 ? (
                    <SkillSectionMemo
                        key={name}
                        name={name}
